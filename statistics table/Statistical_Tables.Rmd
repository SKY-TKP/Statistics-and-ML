...
The entries in the binomial tables are $X\sim\text{Bin}(n,p)$ where $\Pr(X=k)= \binom{n}{k}p^k\ (1-p)^k\ \ ; k=0, 1, 2, .., n$.
...
\newpage
### Table 6.1: Poisson Probabilities.

The entries in the poisson tables are $X\sim\text{Poi}(\lambda)$ where $\Pr(X=k)= \frac{e^{-\lambda}{\lambda}^x}{x!}; k = 0, 1, 2, ..$.
...
\newpage
### Table 6.2: (Continued) Poisson Probabilities.
...
\newpage
### Table 6.3: (Continued) Poisson Probabilities.
